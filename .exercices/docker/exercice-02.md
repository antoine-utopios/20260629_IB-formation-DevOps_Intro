...
```bash
 docker network create exercice-02
```
...
docker run -d \
  ...
  -v exo-02-data:/var/lib/mysql --name exo-02-mysql-server mysql
```
...
```bash
docker run -it --name exo-02-mysql-client ubuntu-mysql
```

* Connexion des deux conteneurs à notre réseau:

```bash
docker connect exercice-02 exo-02-mysql-server
docker connect exercice-02 exo-02-mysql-client
```

* On entre la commande de connexion dans le conteneur client: 

```bash
mysql -h exo-02-mysql-server -u demo -p 
